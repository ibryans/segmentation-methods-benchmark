...
def train(args):
    ...
    running_name = f'{args.dataset}_HRNet_D={args.channel_delta}_F={args.filter}_L={args.loss_function}_B={border_factor}_M={args.batch_size}_P={args.per_val}{"_A" if args.aug else ""}{"_W" if args.class_weights else ""}'
    ...
    if args.resume is None:
        ...
        model = get_sps_cbam_seg_model(n_channels=n_channels, n_classes=n_classes)

        print(f'Creating Model HRNet with {n_channels} input channels, delta={args.channel_delta} and filter={args.filter}')
