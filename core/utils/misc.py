...
def append_filter(filter_name, in_channels=1, device='cpu'):
    filter_dict = {
        'canny'   : (Canny, {'in_channels':in_channels, 'filter':'vertical'}),
        'gabor'   : (Gabor, {'in_channels':in_channels,   'out_channels':16}),
        'sobel'   : (Sobel, {'in_channels':in_channels, 'filter':'vertical'}),
    }
    f_class, f_args = filter_dict[filter_name]
    return f_class(**f_args).to(device)
